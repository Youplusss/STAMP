...
try:
    import seaborn as sns  # optional (used for nicer plots)
...
def plot_history_pred(history, model = "gat",data="swat", out_dir: str | None = None, show: bool = False):
    ...
    out_dir = out_dir or base_dir
    os.makedirs(out_dir, exist_ok=True)
    # include dataset + model to avoid collisions across runs/branches
    plt.savefig(os.path.join(out_dir, f"{data}_{model}_losses_history.pdf"), dpi=200, bbox_inches="tight")
    if show:
        plt.show()
    plt.close(fig)
