...
def get_score_PredAndAE(test_pred_result, test_ae_result, test_generate_result, topk=1, topk_agg='sum', option=1,
                        method="max", alpha=0.4, beta=0.3, gamma=0.3):
    print('=========================** Option: {} **============================\n'.format(str(option)))
    ...
    if isinstance(topk_agg, str) and topk_agg.lower() == 'mean':
        total_topk_err_scores = np.mean(topk_vals, axis=0)
    elif isinstance(topk_agg, str) and topk_agg.lower() == 'max':
        total_topk_err_scores = np.max(topk_vals, axis=0)
    else:
        # default: sum
        total_topk_err_scores = np.sum(topk_vals, axis=0)
...
def get_final_result_POT(test_pred_results, test_ae_results, test_generate_results, train_pred_results,
                         train_ae_results, train_generate_results,
                         y_test_labels, q=1e-4, level=0.9978, topk=1, topk_agg='sum', option=1, method="max", alpha=0.4,
                         beta=0.3, gamma=0.3, search_steps=500):
    # level = 0.9978 for SWaT, 0.9999 for SMD

    test_scores, total_topk_err_scores_test = get_score_PredAndAE(test_pred_results, test_ae_results,
                                                                  test_generate_results, topk=topk, topk_agg=topk_agg,
                                                                  option=option,
                                                                  method=method, alpha=alpha, beta=beta, gamma=gamma)
    train_scores, total_topk_err_scores_train = get_score_PredAndAE(train_pred_results, train_ae_results,
                                                                    train_generate_results, topk=topk,
                                                                    topk_agg=topk_agg, option=option,
                                                                    method=method, alpha=alpha, beta=beta, gamma=gamma)

    # info = pot_eval(np.array(total_topk_err_scores_train), np.array(total_topk_err_scores_test), np.array(y_test_labels), q, level)
    info = bf_search_pot_eval(np.array(total_topk_err_scores_train), np.array(total_topk_err_scores_test),
                              np.array(y_test_labels), q=1e-4, level_range=[0.9999, 1 - 1e-10], step_num=50)
    return info


def get_final_result(test_pred_results, test_ae_results, test_generate_results, y_test_labels, topk=1, topk_agg='sum',
                     option=1, method="max", alpha=0.4, beta=0.3, gamma=0.3, search_steps=500):
    ...
    t, th, predict = bf_search(np.array(total_topk_err_scores), np.array(y_test_labels), start=start, end=end,
                               step_num=search_steps, display_freq=50)
    info = {
        'best-f1': t[0],
        'precision': t[1],
        'recall': t[2],
...
def get_graph_weight(pred_model, nnodes,
                     target_num):  # Obtain feature importance through the weights of edges in the graph structure
