...
class Trainer(object):
    def __init__(self, pred_model, pred_loss, pred_optimizer, ae_model, ae_loss, ae_optimizer, train_loader, val_loader, test_loader, args, scaler, lr_scheduler=None):
        ...
        self.ae_channels = self.args.window_siz e *self.args.nnode s *self.args.in_channels
    ...
    def train_epoch(self, epoch):
        ...
        for batch_m in self.train_loader:
            ...
            if self.args.real_value:
                ...
                generate_batch = torch.from_numpy(generate_batch).float().view(-1, self.args.window_size,
                                                                               self.args.nnodes,
        ...
        loss2 = np.array(loss2_list).mean()
        self.logger.info(
            '**********Train Epoch {}: averaged Loss1: {:.6f}, Loss2: {:.6f}, train_time: {:.3f}s'.format(epoch, loss1,
                                                                                                          loss2,
                                                                                                          end_time - start_time))
        return loss1, loss2
